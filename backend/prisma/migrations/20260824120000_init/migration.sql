...
-- ===========================================================================
-- Hand-written addition. Everything above is generated by Prisma.
--
-- Only ONE statement lives down here, deliberately. Anything Prisma's schema
-- language can express (including the GIN index above) stays under Prisma's
-- management, or it reappears as schema drift on every migrate. A vector
-- index cannot be expressed, because Prisma ignores `Unsupported` columns.
-- ===========================================================================

-- HNSW index over cosine distance, for semantic search (Milestone 1C).
--
-- Cosine because Gemini embeddings are normalised. The operator used at query
-- time (`<=>`) must match `vector_cosine_ops` here, or Postgres silently
-- ignores the index and falls back to a sequential scan over every part --
-- correct results, terrible latency, and no error to tell you.
--
-- m = 16 / ef_construction = 64 are the pgvector defaults: higher means better
-- recall and a slower, hungrier build. Tune against the eval set in 1C rather
-- than by guesswork.
--
-- Building on an empty table is free. When re-embedding the whole catalogue
-- later, DROP this index first, bulk insert, then recreate -- incremental
